...
| OpenAI Codex | Documentation, release readiness, stabilization, repo hygiene, **and all git operations for Claude Code's handed-off work** | [CODEX.md](CODEX.md) |
...
commit/push/tag grant to Claude Code described below. Claude Code must not
run `git commit`, `git push`, `git tag`, or any other git command — read-only
or mutating — in this repository. Claude Code's role stops at editing files
and validating changes; it then hands off to Codex (or Bradley Potts) to
execute any git operation. **OpenAI Codex now executes all git operations in
this repository, including running the actual git commands for work Claude
Code hands off**, in addition to Codex's own documentation/release/hygiene
commits.
...
authority in this roster must push immediately after committing (`git push`, including any needed
